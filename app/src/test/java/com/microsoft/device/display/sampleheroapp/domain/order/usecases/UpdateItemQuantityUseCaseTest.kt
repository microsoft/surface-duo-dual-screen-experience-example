...
class UpdateItemQuantityUseCaseTest {
    ...
    fun deleteItemIfQuantityIsZero() = runBlocking {
        ...
        assertThat(mockRepo.getById(firstOrderEntity.orderId!!)?.items?.size, iz(1))

        updateItemQuantityUseCase.update(mockRepo.autogeneratedItemId, 0)
        assertThat(mockRepo.getById(firstOrderEntity.orderId!!)?.items?.size, iz(0))
    }
    ...
    fun updateItemIfQuantityIsNotZero() = runBlocking {
        ...
        assertThat(mockRepo.getById(firstOrderEntity.orderId!!)?.items?.get(0)?.quantity, iz(newQuantity))
    }
}
